...
const bounds = computed(() => getBbox(props.geoJson) as [number, number, number, number])
...
onMounted(() => {
  if (mapContainer.value) {
    ...
    mapValue.on('mousemove', (data) => {
      ...
      const hoveredBoundFeatures = mapValue.queryRenderedFeatures(point, {
    ...
    mapValue.on('click', (data) => {
      ...
      const clickedBoundFeatures = mapValue.queryRenderedFeatures(point, {
        layers: [BOUNDS_FILL_LAYER_NAME],
      })
...
watchEffect((onCleanup) => {
  if (isDefined(map.value)) {
    const { overlayTileServer } = props

    if (overlayTileServer.type === 'raster') {
      const { raster } = overlayTileServer

      map.value.addSource(OVERLAY_SOURCE_NAME, {
        type: 'raster',
...
watchEffect((onCleanup) => {
  if (isDefined(map.value)) {
    ...
    map.value.addLayer({
      ...
      paint: {
        'line-color': '#ffffff',
        'line-width': ['case', ['boolean', ['feature-state', 'selected'], false], 20, 2],
        'line-opacity': ['case', ['boolean', ['feature-state', 'selected'], false], 0.7, 1],
        'line-offset': ['case', ['boolean', ['feature-state', 'selected'], false], 8, 0],
      },
    })
