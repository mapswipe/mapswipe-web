const projectTypes = {
  ...
  '2': {
    ...
    description: 'Projects of this type consist of several task groups. Each task group has a certain number of features to validate. In our terminology, each feature to validate is one task. In this specific project, the features we validate are ${project.lookFor}. As a user, you can contribute to this project in the following way: Your assignment is to compare the ${project.lookFor} against the background imagery. Does the shape actually correctly outline a ${project.lookFor} in the imagery? If yes, click the "Yes" button or hit number key 1 on your keyboard. If the shape does not match a ${project.lookFor} at all in the image, click "No" or use the number 2 key. If you are not sure or there is cloud cover or any other problem with the image, click "Not sure" or use number 3 key. If the ${project.lookFor} outline is correct, but not properly aligned with the imagery, click "Offset" or use number 4 key. For a clearer view on the underlying imagery, toggle the opacity of the feature overlay by clicking the eye icon. If you get lost on the map, you can click on the "Reset the view" button to zoom to the current shape. Each ${project.lookFor} will be seen and classified by a ${project.verificationNumber} users. The end result for one task is based on the assessment of all the users, so it does not matter that much if one user does not get it right. After you have provided your answer, click the arrow buttons or use the cursor keys to move forward or backward within this task group. Once you have seen and validated all ${project.lookFor} in the task group -- and only then --, you can submit your results (your contribution) by clicking the Save button. After clicking the Save button, you can decide to continue mapping a new task group of this project (unless you have completed all available task groups) or to go back to the project page and select a different project to contribute to. If you want to leave the current project, click on "Home" or "Projects" in the navigation bar and confirm that you want to leave. But be careful -- you will lose all your current answers if you leave before saving.',
  },
  '3': {
    name: 'Compare',
